...
void CircuitBuilder::StartCallTimerInterpreter(GateRef glue,
                                               GateRef gate,
                                               ...
                                               bool useLabel)
{
    ...
    GateRef commentId = args.at(2);
    BRANCH_CIR2(JudgeAotAndFastCall(func, JudgeMethodType::HAS_AOT), &exit, &intCall);
    Bind(&intCall);
    {
        CallNGCRuntime(
            glue, gate, RTSTUB_ID(StartCallTimerWithCommentId), {glue, func, False(), commentId}, useLabel);
        Jump(&exit);
    }
    Bind(&exit);
    SubCfgExit();
...
void CircuitBuilder::StartCallTimerWithCommentId(GateRef glue,
                                                 GateRef gate,
                                                 const std::vector<GateRef>& args,
                                                 bool useLabel)
{
    ...
    {
        CallNGCRuntime(
            glue, gate, RTSTUB_ID(StartCallTimerWithCommentId), {glue, func, isAot, commentId}, useLabel);
        Jump(&exit);
    }
    Bind(&intCall);
    {
...
void CircuitBuilder::EndCallTimerWithCommentId(GateRef glue,
                                               GateRef gate,
                                               const std::vector<GateRef>& args,
                                               bool useLabel)
{
    ...
    Bind(&aotCall);
    {
        CallNGCRuntime(glue, gate, RTSTUB_ID(EndCallTimerWithCommentId), {glue, func, isAot, commentId}, useLabel);
        Jump(&exit);
    }
    Bind(&exit);
    SubCfgExit();
...
void CircuitBuilder::StartCallTimer(GateRef glue, GateRef gate, const std::vector<GateRef>& args, bool useLabel)
{
    ...
    Bind(&aotCall);
    {
        CallNGCRuntime(glue, gate, RTSTUB_ID(StartCallTimer), {glue, func, isAot}, useLabel);
        Jump(&exit);
    }
    Bind(&intCall);
    {
        CallNGCRuntime(glue, gate, RTSTUB_ID(StartCallTimer), {glue, func, False()}, useLabel);
        Jump(&exit);
    }
    Bind(&exit);
    SubCfgExit();
...
void CircuitBuilder::EndCallTimer(GateRef glue, GateRef gate, const std::vector<GateRef>& args, bool useLabel)
{
    ...
    Bind(&aotCall);
    {
        CallNGCRuntime(glue, gate, RTSTUB_ID(EndCallTimer), {glue, func, isAot}, useLabel);
        Jump(&exit);
    }
    Bind(&exit);
    SubCfgExit();
