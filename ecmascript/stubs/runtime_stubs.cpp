...
DEF_RUNTIME_STUBS(UpFrame)
{
    ...
    for (; frameHandler.HasFrame(); frameHandler.PrevJSFrame()) {
        ...
        EndCallTimerWithStrComment(
            argGlue, frameHandler.GetFunction().GetRawData(), false, "ExceptionHandler::UpFrame");
#endif
...
DEF_RUNTIME_STUBS(DeoptHandler)
{
    ...
    EndCallTimerWithStrComment(argGlue, func.GetRawData(), true, "DeoptHandler");
    StartCallTimerWithStrComment(argGlue, func.GetRawData(), false, "DeoptHandler");
#endif
                                                ...
                                                std::string comment)
{
    ...
    timer->StartCount(method, isAot, comment);
}

void RuntimeStubs::EndCallTimerWithStrComment(uintptr_t argGlue, JSTaggedType func, bool isAot, std::string comment)
{
    JSTaggedValue callTarget(func);
    Method* method = Method::Cast(JSFunction::Cast(callTarget)->GetMethod());
    if (method->IsNativeWithCallField()) {
    ...
    auto thread = JSThread::GlueToJSThread(argGlue);
    auto timer = thread->GetEcmaVM()->GetFunctionCallTimer();
    timer->StopCount(method, isAot, comment);
}

void RuntimeStubs::StartCallTimerWithComment(uintptr_t argGlue, JSTaggedType func, bool isAot, uintptr_t comment)
{
...
void RuntimeStubs::EndCallTimerWithComment(uintptr_t argGlue, JSTaggedType func, bool isAot, uintptr_t comment)
{
    const char* str = reinterpret_cast<const char*>(comment);
    const char* tag = str == nullptr ? "unknown" : str;
...
void RuntimeStubs::EndCallTimer(uintptr_t argGlue, JSTaggedType func, bool isAot)
{
